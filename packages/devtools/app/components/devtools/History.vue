...
<template>
  <div v-if="stats">
    ...
    <div v-else class="flex flex-col-reverse p-1 gap-1">
      <DevtoolsHistoryItem
        v-for="(item, index) in showCacheOps ? stats.history : stats.history.filter((historyItem) => !historyItem.operation.startsWith('cache') && historyItem.operation !== 'itemGarbageCollect')"
        :key="index"
        :item
      />
    </div>
