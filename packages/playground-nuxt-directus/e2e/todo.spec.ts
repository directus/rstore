...
test('can edit a todo title', async ({ page }) => {
  const originalText = uniqueText('playwright-directus-edit')
...
test('can delete a todo', async ({ page }) => {
  const todoText = uniqueText('playwright-directus-delete')
...
test('can create multiple todos in sequence', async ({ page }) => {
  const todoTextA = uniqueText('playwright-directus-seq-a')
  const todoTextB = uniqueText('playwright-directus-seq-b')
