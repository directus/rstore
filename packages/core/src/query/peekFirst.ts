    ...
    store.$hooks.callHookSync('cacheFilterFirst', {
      ...
      readItemsFromCache: (options) => {
        ...
        let items = store.$cache.readItems({
          model,
          marker: force ? undefined : getMarker('first', marker),
          filter: options?.applyFilter && typeof findOptions?.filter === 'function' ? findOptions.filter as (item: ResolvedModelItemBase<TModel, TModelDefaults, TModelList>) => boolean : undefined,
        }) ?? []
