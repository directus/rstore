...
>({
  ...
  function getPageId(optionsExtension: Partial<TOptions>, index: number): string {
    return `${queryId.value}#${index}`
  }

  function createPage(optionsExtension: Partial<TOptions>, main: boolean): VueQueryPage<TCollection, TCollectionDefaults, TSchema, TOptions, TResult> {
  ...
  function fetchMore(optionsExtension: Partial<TOptions>): HybridPromise<{ page: VueQueryPage<TCollection, TCollectionDefaults, TSchema, TOptions, TResult> }> {
        ...
        if (!forceFetch && fetchPolicy === 'cache-and-fetch') {
          ...
          fetchMethod({
            ...
            if (valid && queryTracking && newQueryTracking2) {
              queryTracking.handleQueryTracking(newQueryTracking2, backgroundResult, finalOptions.include)
            }
          })
        }
        ...
        if (finalOptions.fetchPolicy !== 'no-cache' && finalOptions.fetchPolicy !== 'fetch-only' && cache._private.state.pageRefs.has(page.id)) {
          ...
          if (valid) {
            ...
            if (queryTrackingEnabled && shouldHandleQueryTracking && queryTracking && newQueryTracking) {
              queryTracking.handleQueryTracking(newQueryTracking, undefined, finalOptions.include, page.main)
            }
          }
        }
      }
