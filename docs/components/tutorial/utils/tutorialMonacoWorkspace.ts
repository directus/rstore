...
export async function ensureTutorialMonacoWorkspace() {
  monacoApi ??= await import('./loadMonaco').then(({ loadMonaco }) => loadMonaco())
...
class TutorialLspConnection {
  ...
  async initialize() {
    ...
    let result = await this.request<LspInitializeResult>('initialize', initializeParams)

    if (!result?.capabilities) {
      await new Promise(resolve => window.setTimeout(resolve, 200))
      result = await this.request<LspInitializeResult>('initialize', initializeParams)
    }
