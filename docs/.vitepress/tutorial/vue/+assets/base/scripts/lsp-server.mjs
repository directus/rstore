...
const typescriptSdk = `${workspaceRoot}/node_modules/typescript/lib`
const pluginFileTypes = ['vue', 'javascript', 'typescript']
const pluginLocationCandidates = [
  `${workspaceRoot}/node_modules/@vue/language-server/node_modules/@vue/typescript-plugin`,
...
const languageServer = spawn(process.execPath, [
  './scripts/vue-language-server.mjs',
  '--stdio',
  `--tsdk=${typescriptSdk}`,
], {
  cwd: workspaceRoot,
...
const typescriptServer = spawn(process.execPath, [
  './scripts/typescript-language-server.mjs',
  '--stdio',
], {
  cwd: workspaceRoot,
  stdio: ['pipe', 'pipe', 'pipe'],
...
languageServer.stderr.on('data', (chunk) => {
  writeBrowserMessage({
    method: '$/bridgeLog',
    params: `[vue-ls] ${chunk.toString()}`,
  })
})

typescriptServer.stderr.on('data', (chunk) => {
  writeBrowserMessage({
    method: '$/bridgeLog',
    params: `[ts-ls] ${chunk.toString()}`,
  })
})
...
languageServer.on('exit', (code, signal) => {
  writeBrowserMessage({
    method: '$/bridgeLog',
    params: `[vue-ls] exited with code ${String(code)}${signal ? ` signal ${signal}` : ''}`,
  })
  typescriptServer.kill()
  process.exit()
})

typescriptServer.on('exit', (code, signal) => {
  writeBrowserMessage({
    method: '$/bridgeLog',
    params: `[ts-ls] exited with code ${String(code)}${signal ? ` signal ${signal}` : ''}`,
  })
  languageServer.kill()
  process.exit()
})
...
async function forwardTypeScriptRequest(params) {
  const normalizedParams = Array.isArray(params?.[0]) ? params[0] : params
  const [requestId, command, args] = Array.isArray(normalizedParams) ? normalizedParams : []
