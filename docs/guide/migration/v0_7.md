...
Since it now contains both models and relations ([see below](#relation-definitions-shape-changed)) - and maybe other objects in the future, the `models` option has been renamed to `schema`.
:::
...
The types for `FindOptions` changed, so you need to update your type augmentations:
...
declare module '@rstore/vue' {
  ...
  > extends FindOptionsBase<TModel, TModelDefaults, TSchema> { // [!code ++]
