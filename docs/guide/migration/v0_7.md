...
<template>
  <UAlert
    v-if="auth.login.$error.value" // [!code --]
    v-if="auth.login.$error" // [!code ++]
    :description="auth.login.$error.value.message" // [!code --]
    :description="auth.login.$error.message" // [!code ++]
    color="error"
  />
  <UButton
    type="submit"
    :loading="auth.login.$loading.value" // [!code --]
    :loading="auth.login.$loading" // [!code ++]
  />
</template>
```
